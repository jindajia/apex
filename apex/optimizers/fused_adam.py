...
class FusedAdam(torch.optim.Optimizer):
    ...
    def no_update_mv_step(self, closure=None, grads=None, output_params=None, scale=None, grad_norms=None, grad_scaler=None):
        """Performs a single optimization step.
        ...
        for group, group_master in zip(self.param_groups, self.param_groups_master):
            ...
            # per parameter step can be easily support by making it tensor, or pass list into kernel
            if 'step' in group:
                new_group_step = group['step'] + 1 if not self.capturable else group['step'] + (self._dummy_overflow_buf != 1).to(torch.int)
            else:
                new_group_step = 1 if not self.capturable else torch.tensor([1], dtype=torch.int, device=device)
            ...
            if self.capturable:
                ...
                if len(g_16) > 0:
                    multi_tensor_applier(self.multi_tensor_adam_noupdate_mv_capturable_master if self.master_weights
                            else self.multi_tensor_adam_noupdate_mv_capturable,
                            self._dummy_overflow_buf,
                            [g_16, p_16, m_16, v_16, p_16_master] if self.master_weights
                            else [g_16, p_16, m_16, v_16],
                            group['lr'],
                ...
                if len(g_bf) > 0:
                    multi_tensor_applier(
                            self.multi_tensor_adam_noupdate_mv_capturable,
                            self._dummy_overflow_buf,
                            [g_bf, p_bf, m_bf, v_bf],
                            group['lr'],
                            beta1,
                ...
                if len(g_32) > 0:
                    multi_tensor_applier(self.multi_tensor_adam_noupdate_mv_capturable_master if self.master_weights
                            else self.multi_tensor_noupdate_mv_adam_capturable,
                            self._dummy_overflow_buf,
                            [g_32, p_32, m_32, v_32, p_32_master] if self.master_weights
                            else [g_32, p_32, m_32, v_32],
                            group['lr'],
                ...
                if len(g_16) > 0:
                    multi_tensor_applier(self.multi_tensor_adam_noupdate_mv,
                            self._dummy_overflow_buf,
                            [g_16, p_16, m_16, v_16],
                            group['lr'],
                            beta1,
                ...
                if len(g_bf) > 0:
                    multi_tensor_applier(
                            self.multi_tensor_adam_noupdate_mv,
                            self._dummy_overflow_buf,
                            [g_bf, p_bf, m_bf, v_bf],
                            group['lr'],
                            beta1,
